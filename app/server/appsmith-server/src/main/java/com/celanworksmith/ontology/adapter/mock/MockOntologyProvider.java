...
public class MockOntologyProvider implements OntologyProvider {
    private static final List<ObjectTypeDTO> OBJECT_TYPES = List.of(
            new ObjectTypeDTO("Supplier", "Supplier", List.of(
            ...
            new ObjectTypeDTO("PurchaseOrder", "Purchase Order", List.of(
            ...
            new ObjectTypeDTO("ProductionOrder", "Production Order", List.of(
            ...
            new ObjectTypeDTO("DeliveryOrder", "Delivery Order", List.of(
                    property("purchaseOrderId", "Purchase Order", "REFERENCE", true, false, false),
                    property("status", "Status", "ENUM", true, false, false),
            ...
            new ObjectTypeDTO("FinancialRecord", "Financial Record", List.of(
                    property("purchaseOrderId", "Purchase Order", "REFERENCE", true, false, false),
            ...
            new ObjectTypeDTO("SupplierRating", "Supplier Rating", List.of(
                    property("supplierId", "Supplier", "REFERENCE", true, false, false),
    ...
    private static final List<LinkTypeDTO> LINK_TYPES = List.of(
            ...
            new LinkTypeDTO("po_production", "Purchase Order Production", "PurchaseOrder", "ProductionOrder", "ONE_TO_ONE"),
    ...
    private static final List<FunctionDTO> FUNCTIONS = List.of(
            new FunctionDTO("CalculateDelayDays", "Calculate Delay Days", "INTEGER", List.of(parameter("poId", "Purchase Order", "STRING")), true),
            new FunctionDTO("CalculatePenalty", "Calculate Penalty", "DECIMAL", List.of(parameter("poId", "Purchase Order", "STRING")), true),
            new FunctionDTO("CalculateAdjustedProfit", "Calculate Adjusted Profit", "DECIMAL", List.of(parameter("poId", "Purchase Order", "STRING")), true),
            new FunctionDTO("CalculateMarginRate", "Calculate Margin Rate", "DECIMAL", List.of(parameter("poId", "Purchase Order", "STRING")), true),
            new FunctionDTO("CalculateSupplierGrade", "Calculate Supplier Grade", "DECIMAL", List.of(parameter("supplierId", "Supplier", "STRING")), true));

    private static final List<ActionTypeDTO> ACTIONS = List.of(
            new ActionTypeDTO("UpdateProductionSchedule", "Update Production Schedule", "PurchaseOrder", List.of(parameter("newScheduleDate", "New Schedule Date", "DATETIME")), true),
            new ActionTypeDTO("UpdateDeliveryDate", "Update Delivery Date", "PurchaseOrder", List.of(parameter("newDeliveryDate", "New Delivery Date", "DATETIME")), true),
            new ActionTypeDTO("UpdateFinancialRecord", "Update Financial Record", "PurchaseOrder", List.of(parameter("penalty", "Penalty", "DECIMAL")), true),
            new ActionTypeDTO("NotifyProductionTeam", "Notify Production Team", "PurchaseOrder", List.of(parameter("message", "Message", "STRING")), false));

    private static PropertyDTO property(String id, String displayName, String dataType, boolean required, boolean readOnly, boolean derived) {
    ...
    @Override
    public Mono<List<ObjectTypeDTO>> getObjectTypes() { return Mono.just(OBJECT_TYPES); }
    ...
    public Mono<List<LinkTypeDTO>> getLinkTypes(String sourceTypeId) {
        ...
        return Mono.just(LINK_TYPES.stream().filter(link -> link.sourceTypeId().equals(sourceTypeId)).toList());
    }

    @Override
    public Mono<List<FunctionDTO>> getFunctions() { return Mono.just(FUNCTIONS); }
    ...
    public Mono<List<ActionTypeDTO>> getActions(String objectTypeId) {
        ...
        return Mono.just(ACTIONS.stream().filter(action -> action.objectTypeId().equals(objectTypeId)).toList());
    }
}
